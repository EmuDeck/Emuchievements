...
export class AchievementManager implements Manager
{
	...
	public clearCache()
	{
		localforage.clear().then(_ => this.logger.debug('Cleared cache'));
	...
	fetchAchievements(app_id: number): FetchedAchievements
	{
		...
		if (!achievementsData?.data)
		{
			...
			this.throttle(async () => {
				try
				{
					const result = await this.getAchievementsForGame(app_id)
						   .then((result: AchievementsData | undefined) => {
							   if (result != undefined)
							   {
								   return this.processRetroAchievements(result);
							   } else
							   {
								   return loadingFetchedAchievements;
							   }
	...
	processRetroAchievements(retro: AchievementsData): FetchedAchievements
	{
		...
		const { all, global } = Object.entries(achievements)
			   .reduce((result, [_, achievement]) => {
				   ...
				   if (result.all.data && result.global.data)
				   {
					   steam.bAchieved ?
							 result.all.data.achieved[steam.strID] = steam :
							 result.all.data.unachieved[steam.strID] = steam;

					   result.global.data[steam.strID] = ((achievement.numAwarded || 0) / (numDistinctPlayersCasual || 1)) * 100.0;
				   }

				   return result;
			   }, { all: defaultAchievements, global: defaultGlobalAchievements });
	...
	{
		return new Promise<FetchedAchievements | undefined>(async (resolve) => {
	...
	async refresh_achievements_for_apps(app_ids: number[]): Promise<void>
	{
		...
		this.processed = 0;
		await Promise.map(app_ids, (async (app_id: number) => await this.refresh_achievements_for_app(app_id)), {
			concurrency: 3,
		});
